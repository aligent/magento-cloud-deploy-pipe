...
push_to_secondary_remote() {
     ...
     git remote add secondary-remote ${SECONDARY_REMOTE}
     ...
     git push secondary-remote ${BITBUCKET_BRANCH}  2>&1 | tee /dev/stderr | grep -iq "Deployment completed"
}
